	...
	*/
	...
	Uploader.prototype = {
		...
		submit:function(){
			...
			http.get(self.getPath(),self.getParams(),function(result){
				var data = result.data;

				self.signature.value = data.signature;//必要的签名
				self.policy.value = data.policy;//必要的策略
				self.domain = 'http://' + data.domain;//文件存储的域名
				form.setAttribute('action',data.action);//设置form上传action
				form.submit();
		...
		createHiddenField:function(){
			...
			if(!signature){
				...
				form.appendChild(self.signature);
			}else{
